...
#' @param strategies        Which strategies to plot?
#' @param cumulative        Should cumulative probabilities be shown?
#' @param alpha             The nominal alpha to plot.
...
plotFalsePositiveNegatives <- function(confusionMatrices,
                                       strategies = c("Calibrated MaxSPRT", "MaxSPRT", "P"),
                                       cumulative = TRUE,
                                       alpha = 0.05,
                                       fileName = NULL) {
  ...
  plotData <- confusionMatrices %>%
    filter(.data$label %in% strategies) %>%
